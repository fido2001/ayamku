    ...
    <div class="card-body">
        ...
        @if ($order->bukti == null)
            <div class="alert alert-danger d-flex justify-content-between align-items-center" role="alert">
                ...
                <div><a href="{{ route('produk.pembayaran.distributor', $order->id) }}" class="btn btn-warning">Bayar Pesanan Disini</a></div>
            </div>
        @endif
        <h5 class="card-title">Nama Produk : {{ $order->nama_produk }}</h5>
        <h5 class="card-title">Tanggal Pemesanan : {{ Carbon\Carbon::parse($order->tanggal)->translatedFormat('l, d F Y') }}</h5>
        ...
        <h6 class="card-text">Status : {{ $order->status_order }}</h6>
